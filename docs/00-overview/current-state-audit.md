...
| S3 | **Two spacing scales that contradict each other.** `spacing/3xs = 8px` but `Spacing/Sizes/3xs = 2px`; `spacing/xs = 12px` but `Spacing/Sizes/lg = 12px` | Button uses `spacing/xs`, TextField uses `Spacing/Sizes/lg` for the same 12px | Same value, two names; migrations break | One `sys.space` scale; alias table in `docs/02-styles/spacing.md` |
...
| S6 | **Components missing from Figma but present in Storybook / production.** Modal, Panel, Tooltip (only implied by `checkboxfield --disabled-info-tooltip`), Toast | Storybook has Modal, Panel, Tooltip | Engineers build from code, designers from nothing | Added to catalog as `proposed`; design from the spec |
...
| S11 | **Elevation is undefined.** No shadow styles or variables | `03-Colors`, `04-Token` only | Cards, menus and sheets ship with ad-hoc shadows | Add `sys.elevation.0-3` styles |
| S12 | **Some hard-coded values escape the tokens.** Status pill radius `30px`, Alertbox `gap 4px` without a variable, feature button gradient stops | design context on `Status`, `Alertbox`, `Button --feature` | Theme changes miss them | Map to `sys.shape.full`, `sys.space.2xs`, `sys.color.feature.*` |
...
| Styles: elevation | absent | proposed tokens |
...
| Components: tooltip | implied | proposed spec |
